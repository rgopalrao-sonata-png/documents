...
A recurring question is: *If most LPR data already flows through the Snowflake → S3 → Aurora batch pipeline, why does `course_progress` bypass that path and get enriched separately?* The short answer is that `course_progress` is not just another warehouse field. It is a learner-facing computed value with stricter provenance requirements than the rest of the report. The current design is the result of two earlier failed approaches and an explicit decision to prefer source-of-truth fidelity over architectural uniformity.

### The Standard Pipeline Has a ~1-Day Lag

The batch pipeline (dbt → S3 → Aurora) is optimized for throughput and operational simplicity, not freshness. In practice, it introduces roughly a one-day lag between what a learner sees in the LMS and what an enterprise admin sees in the LPR. For most fields — enrollment metadata, grades, user details, subsidy context — that delay is acceptable. For `course_progress`, however, the data is both highly visible and easy for customers to cross-check against the LMS, so freshness and correctness matter more:
...
Ticket [ENT-5795](https://2u-internal.atlassian.net/browse/ENT-5795) explored deriving progress from block-level completion data already available in Snowflake. That approach failed because `course_progress` is not a raw source field; it is a **computed LMS output**. The LMS applies its own rules around block weighting, exclusions, visibility, and course structure when calculating the learner-visible percentage. Re-implementing that logic in dbt SQL produced values that diverged from what learners actually saw in-course. Even a small mismatch (~1–2%) is operationally expensive here, because enterprise admins compare the report directly with learner screenshots and learner-reported values. Once we accepted that we could not reproduce the LMS calculation reliably, this option stopped being viable.

### Attempt 2 (ENT-9207) — Call the LMS API Directly

The next option considered in [ENT-9207](https://2u-internal.atlassian.net/browse/ENT-9207) was to call the LMS endpoint that exposes the same progress value shown to the learner:

```
{LMS_BASE_URL}/api/course_home/progress/{courseId}/{targetUserId}/
```

Conceptually, this was attractive because it would have guaranteed exact alignment with the learner experience. In practice, it also failed for architectural reasons:
...
So while the endpoint was correct from a data-fidelity standpoint, it was not a safe or scalable integration pattern for enterprise reporting.

### Why Real-Time Snowflake Is the Right Answer

The breakthrough in [ENT-11183](https://2u-internal.atlassian.net/browse/ENT-11183) and [DPSD-8550](https://2u-internal.atlassian.net/browse/DPSD-8550) was to have the Data Platform team surface the already-calculated LMS value directly into `PROD.ENTERPRISE.LEARNER_PROGRESS_REPORT_INTERNAL`. That changed the problem completely: we no longer had to either recreate LMS logic or call LMS synchronously at request time. We could read the canonical computed value from Snowflake, which solves the two earlier blockers simultaneously:
...
| Can't call the LMS API at scale | Data Platform handles upstream collection and publishing; the application only reads the result |
...
The reason we still do **not** copy `course_progress` through the full Aurora path is pragmatic: that would introduce another daily replication hop for the single field where an additional delay is least acceptable. By querying Snowflake at request time, the application serves the freshest value currently published by Data Platform, without adding another copy layer. Put differently: Aurora is the right store for stable batch-reporting fields; Snowflake is the right read path for this one high-sensitivity, source-of-truth metric.
...
These are operational log lines, not 1:1 SQL statement counts — a single request can generate multiple connection and query log entries. Even so, they are directionally useful: the traffic is consistent with interactive Admin Portal usage (page loads, pagination, CSV exports) plus any consumer polling. The planned caching work in `ENT0-9531` should materially reduce repeat reads within the same refresh window without changing the underlying data contract.
...
The diagram below shows the **previous / standard flow** for LPR fields that move from Snowflake into Aurora before they are served by the API:

![Previous Snowflake to Aurora LPR flow](lpr-previous-batch-flow.svg)
