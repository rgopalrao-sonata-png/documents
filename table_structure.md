...
# Part 3: Database Table Reference — Multi-License Subscription System

> **Services covered:** `license-manager` · `enterprise-catalog`
> **DB engine:** MySQL 8.0 (devstack port `3406`)
> **All models extend:** `TimeStampedModel` → implicit `created` + `modified` datetime columns on every table

---

## Table Order (Dependency / Join Path)

```
1. subscriptions_customeragreement       (license-manager DB)
2. subscriptions_subscriptionplan        (license-manager DB)
3. subscriptions_subscriptionplanrenewal (license-manager DB)
4. subscriptions_license                 (license-manager DB)
5. subscriptions_licenseevent            (license-manager DB)
   ─────────────────────────────────────────────────────────
6. catalog_catalogquery                  (enterprise-catalog DB)
7. catalog_enterprisecatalog             (enterprise-catalog DB)
8. catalog_contentmetadata               (enterprise-catalog DB)
9. catalog_contentmetadata_catalog_queries  (M2M join — enterprise-catalog DB)
10. catalog_restrictedcoursemetadata     (enterprise-catalog DB)
```

The **join across services** is: `subscriptions_subscriptionplan.enterprise_catalog_uuid`
→ `catalog_enterprisecatalog.uuid`

---

## 1. `subscriptions_customeragreement`

**Django model:** `CustomerAgreement`
**App:** `license_manager.apps.subscriptions`
**Role:** Top-level anchor binding an Enterprise to all its subscription plans.

| Column | Type | Nullable | Key | Description |
|---|---|---|---|---|
| `uuid` | CHAR(32) | NO | PK | Agreement UUID — primary key |
| `enterprise_customer_uuid` | CHAR(32) | NO | **UQ** | UUID of the enterprise — links to LMS enterprise customer |
| `enterprise_customer_slug` | VARCHAR(128) | YES | UQ | Human-readable slug (e.g. `knotion`) |
| `enterprise_customer_name` | VARCHAR(255) | YES | — | Display name |
| `default_enterprise_catalog_uuid` | CHAR(32) | YES | — | Fallback catalog UUID if a plan has no explicit catalog |
| `disable_expiration_notifications` | TINYINT(1) | NO | — | Suppress expiry emails |
| `disable_onboarding_notifications` | TINYINT(1) | NO | — | Suppress assignment/activation emails |
| `license_duration_before_purge` | BIGINT | NO | — | Microseconds before stale licenses are purged |
| `has_custom_license_expiration_messaging` | TINYINT(1) | NO | — | Show custom expiry modal |
| `modal_header_text` | VARCHAR(512) | YES | — | Header text for expiry modal |
| `expired_subscription_modal_messaging` | LONGTEXT | YES | — | Body text for expiry modal |
| `button_label_in_modal` | VARCHAR(255) | YES | — | CTA button label |
| `url_for_button_in_modal` | VARCHAR(512) | YES | — | CTA button URL |
| `enable_auto_applied_subscriptions_with_universal_link` | TINYINT(1) | NO | — | Auto-apply via Universal Link (not just SSO) |
| `enable_auto_scaling_of_current_plan` | TINYINT(1) | NO | IDX | Allow plan to auto-add seats |
| `auto_scaling_max_licenses` | INT | YES | — | Hard cap on auto-scale seat count |
| `auto_scaling_threshold_percentage` | SMALLINT | YES | — | Trigger threshold (% allocated) |
| `auto_scaling_increment_percentage` | SMALLINT | YES | — | Seats added on each scale event (%) |
| `created` | DATETIME(6) | NO | — | Row creation timestamp |
| `modified` | DATETIME(6) | NO | — | Row last-modified timestamp |

**Key relationships:**
- `1:N` → `subscriptions_subscriptionplan` (via `customer_agreement_id` FK)
- `1:1` → LMS `enterprise_customer` (cross-service, by `enterprise_customer_uuid`)

**Multi-license relevance:**
`default_enterprise_catalog_uuid` is used as the fallback catalog when a plan's `enterprise_catalog_uuid` is NULL.
`available_subscription_catalogs` property aggregates all active plan catalog UUIDs — used by the frontend to pre-screen catalog membership before calling `resolveApplicableSubscriptionLicense`.

---

## 2. `subscriptions_subscriptionplan`

**Django model:** `SubscriptionPlan`
**App:** `license_manager.apps.subscriptions`
**Role:** One subscription purchase. Each plan covers exactly **one** enterprise catalog. A customer may have many plans → multi-license.

| Column | Type | Nullable | Key | Description |
|---|---|---|---|---|
| `uuid` | CHAR(32) | NO | PK | Plan UUID |
| `title` | VARCHAR(128) | NO | UQ† | Human-readable plan name |
| `start_date` | DATETIME(6) | NO | — | Plan validity start |
| `expiration_date` | DATETIME(6) | NO | — | Plan validity end |
| `expiration_processed` | TINYINT(1) | NO | — | Whether expiry job has run |
| **`enterprise_catalog_uuid`** | CHAR(32) | NO | — | **FK (cross-service) → `catalog_enterprisecatalog.uuid`** |
| `customer_agreement_id` | CHAR(32) | NO | FK | → `subscriptions_customeragreement.uuid` |
| `is_active` | TINYINT(1) | NO | — | Admin-controlled active flag |
| `is_revocation_cap_enabled` | TINYINT(1) | NO | — | Whether revocation limit is enforced |
| `revoke_max_percentage` | SMALLINT | NO | — | Max % of licenses that can be revoked |
| `num_revocations_applied` | INT UNSIGNED | NO | — | Running count of revocations |
| `salesforce_opportunity_id` | VARCHAR(18) | YES | — | Deprecated SF opportunity ID |
| `salesforce_opportunity_line_item` | VARCHAR(18) | YES | — | SF Opportunity Line Item ID |
| `product_id` | INT | YES | FK | → `subscriptions_product.id` |
| `for_internal_use_only` | TINYINT(1) | NO | — | Test/internal plan flag |
| `can_freeze_unused_licenses` | TINYINT(1) | NO | — | Allow license freeze |
| `last_freeze_timestamp` | DATETIME(6) | YES | — | When last frozen |
| `should_auto_apply_licenses` | TINYINT(1) | YES | — | Auto-apply licenses on SSO/Universal Link |
| `desired_num_licenses` | INT UNSIGNED | YES | — | Target seat count (asynchronously provisioned) |
| `created` | DATETIME(6) | NO | — | Creation timestamp |
| `modified` | DATETIME(6) | NO | — | Last-modified timestamp |
...
| Property | Logic |
|---|---|
| `is_current` | `start_date <= now() <= expiration_date` |
| `num_licenses` | `COUNT(licenses)` |
| `unassigned_licenses` | `licenses WHERE status='unassigned'` |
| `assigned_licenses` | `licenses WHERE status='assigned'` |
| `activated_licenses` | `licenses WHERE status='activated'` |
| `revoked_licenses` | `licenses WHERE status='revoked'` |

**Multi-license relevance:**
`enterprise_catalog_uuid` is the **critical link** between this plan and `catalog_enterprisecatalog`.
When a learner has two plans → two different `enterprise_catalog_uuid` values → two separate course sets → multi-license.

---

## 3. `subscriptions_subscriptionplanrenewal`

**Django model:** `SubscriptionPlanRenewal`
**Role:** Links an old plan (`prior_subscription_plan`) to its future renewal plan (`renewed_subscription_plan`).

| Column | Type | Nullable | Key | Description |
|---|---|---|---|---|
| `id` | INT | NO | PK | Auto-increment ID |
| `prior_subscription_plan_id` | CHAR(32) | NO | FK, UQ | → `subscriptions_subscriptionplan.uuid` (the expiring plan) |
| `renewed_subscription_plan_id` | CHAR(32) | YES | FK, UQ | → `subscriptions_subscriptionplan.uuid` (the new plan) |
| `number_of_licenses` | INT UNSIGNED | NO | — | Seat count for the renewed plan |
| `effective_date` | DATETIME(6) | NO | — | When renewal takes effect |
| `renewed_expiration_date` | DATETIME(6) | NO | — | New plan expiration |
| `processed` | TINYINT(1) | NO | — | Whether renewal job has run |
| `processed_datetime` | DATETIME(6) | YES | — | When the renewal was processed |
| `renewed_plan_title` | VARCHAR(128) | YES | — | Title override for new plan |
| `license_types_to_copy` | VARCHAR(32) | NO | — | `ASSIGNED_AND_ACTIVATED`, `ALL`, or `NONE` |
| `disable_auto_apply_licenses` | TINYINT(1) | NO | — | Disable auto-apply on renewed plan |
| `exempt_from_batch_processing` | TINYINT(1) | NO | — | Skip cron processing |
| `salesforce_opportunity_id` | VARCHAR(18) | YES | — | SF Line Item for renewal |
| `created` | DATETIME(6) | NO | — | — |
| `modified` | DATETIME(6) | NO | — | — |

**Multi-license relevance:** Renewals chain plans together. If a learner had License A on Plan 1 and Plan 1 is renewed to Plan 2, the license is copied to Plan 2. In a multi-license world, the learner may simultaneously have a license on the renewed and the original plan during the transition window.

---

## 4. `subscriptions_license`

**Django model:** `License`
**App:** `license_manager.apps.subscriptions`
**Role:** Individual seat — the unit that is assigned, activated, and tracked per user per plan.
This is the **central table** for multi-license: one user can now have multiple rows here (one per plan).

| Column | Type | Nullable | Key | Description |
|---|---|---|---|---|
| `uuid` | CHAR(32) | NO | PK | License UUID |
| `status` | VARCHAR(25) | NO | IDX† | `unassigned` · `assigned` · `activated` · `revoked` |
| **`user_email`** | VARCHAR(254) | YES | IDX | Assigned learner email (PII) |
| **`lms_user_id`** | INT | YES | IDX | LMS user integer ID (set on activation) |
| **`subscription_plan_id`** | CHAR(32) | NO | FK | → `subscriptions_subscriptionplan.uuid` |
| `activation_key` | CHAR(32) | YES | — | UUID4 token sent in activation email link |
| `activation_date` | DATETIME(6) | YES | — | When learner clicked the activation link |
| `assigned_date` | DATETIME(6) | YES | — | When admin assigned this license |
| `last_remind_date` | DATETIME(6) | YES | — | Last reminder email sent |
| `expiration_reminder_sent_date` | DATETIME(6) | YES | — | Expiry reminder email sent |
| `revoked_date` | DATETIME(6) | YES | — | When license was revoked |
| `renewed_to_id` | CHAR(32) | YES | FK | → `subscriptions_license.uuid` (self-referential; renewal copy) |
| `auto_applied` | TINYINT(1) | YES | — | Whether license was auto-applied (SSO/Universal Link) |
| `created` | DATETIME(6) | NO | — | Creation timestamp |
| `modified` | DATETIME(6) | NO | — | Last-modified timestamp |

† Composite index: `(subscription_plan_id, status)` → `subscription_plan_status_idx`

**Constraint (enforced in `License.clean()`):**
> A user (`user_email`) may only have ONE `assigned` or `activated` license **per subscription plan**.
> Cross-plan: a user CAN have multiple `activated` licenses as long as they are in different plans.
> This is the **constraint that enables multi-license** — one row per plan per user is allowed.

**Status lifecycle:**

```
UNASSIGNED ──assign()──► ASSIGNED ──activate()──► ACTIVATED
                                                       │
                                                    revoke()
                                                       │
                                                       ▼
                                                   REVOKED ──reset_to_unassigned()──► UNASSIGNED
```

**Multi-license join queries:**

```sql
-- Find all activated licenses for a user across ALL plans (multi-license)
SELECT
    l.uuid                              AS license_uuid,
    l.status,
    l.activation_date,
    sp.uuid                             AS plan_uuid,
    sp.title                            AS plan_title,
    sp.enterprise_catalog_uuid,
    sp.expiration_date,
    sp.is_active
FROM subscriptions_license l
JOIN subscriptions_subscriptionplan sp ON sp.uuid = l.subscription_plan_id
JOIN subscriptions_customeragreement ca ON ca.uuid = sp.customer_agreement_id
WHERE l.user_email     = 'maria@knotion.com'
  AND l.status         = 'activated'
  AND ca.enterprise_customer_uuid = 'enterprise-uuid-knotion'
  AND sp.is_active = 1
ORDER BY l.activation_date ASC;
```

**Expected result (Knotion example):**

| license_uuid | status | activation_date | plan_title | enterprise_catalog_uuid | expiration_date |
|---|---|---|---|---|---|
| `lic-A` | activated | 2025-01-15 | Data Science Essentials | `cat-data-science` | 2026-12-31 |
| `lic-B` | activated | 2025-04-01 | Managerial Leadership | `cat-leadership` | 2026-12-31 |

---

## 5. `subscriptions_licenseevent`

**Django model:** `LicenseEvent`
**Role:** Audit log of license status transitions. Not used in course eligibility but important for support tooling.

| Column | Type | Nullable | Key | Description |
|---|---|---|---|---|
| `id` | INT | NO | PK | Auto-increment ID |
| `license_id` | CHAR(32) | NO | FK | → `subscriptions_license.uuid` |
| `event_name` | VARCHAR(255) | NO | — | Event type (e.g. `license_assigned`, `license_activated`) |
| `event_data` | LONGTEXT | YES | — | JSON payload with event metadata |
| `timestamp` | DATETIME(6) | NO | IDX | When the event occurred |
| `created` | DATETIME(6) | NO | — | — |
| `modified` | DATETIME(6) | NO | — | — |

---

## 6. `catalog_catalogquery`

**Django model:** `CatalogQuery`
**App:** `enterprise_catalog.apps.catalog`
**Role:** Reusable query definition that determines which courses belong to a catalog.

| Column | Type | Nullable | Key | Description |
|---|---|---|---|---|
| `id` | INT | NO | PK | Auto-increment ID |
| `uuid` | CHAR(32) | NO | UQ | Query UUID |
| `title` | VARCHAR(100) | YES | UQ | Human-readable name |
| `content_filter` | LONGTEXT (JSON) | NO | — | JSONField: Elasticsearch filter parameters defining catalog content |
| `content_filter_hash` | VARCHAR(32) | YES | UQ | MD5 of `content_filter` — used for deduplication |
| `created` | DATETIME(6) | NO | — | — |
| `modified` | DATETIME(6) | NO | — | — |

**`content_filter` example (Data Science catalog):**
```json
{
    "content_type": ["course"],
    "subjects": ["Data Analysis & Statistics", "Python"],
    "organizations": ["MIT", "IBM"],
    "level_type": ["Intermediate", "Advanced"]
}
```

**`content_filter` example (Leadership catalog — CLOSED: specific course keys only):**
```json
{
    "content_type": ["course"],
    "key": [
...
**Multi-license relevance:** A CLOSED catalog query with specific `key` values is exactly how Knotion's Learning Pathways are implemented — only those exact course keys appear in the catalog, preventing access to other content even within the same enterprise.

---

## 7. `catalog_enterprisecatalog`

**Django model:** `EnterpriseCatalog`
**App:** `enterprise_catalog.apps.catalog`
**Role:** Binds a `CatalogQuery` to an enterprise customer, creating a named catalog. This UUID is what `subscriptions_subscriptionplan.enterprise_catalog_uuid` references.

| Column | Type | Nullable | Key | Description |
|---|---|---|---|---|
| `uuid` | CHAR(32) | NO | PK | **Catalog UUID** — referenced by `subscriptions_subscriptionplan.enterprise_catalog_uuid` |
| `title` | VARCHAR(255) | NO | — | Catalog display name (e.g. "Data Science Essentials Catalog") |
| `enterprise_uuid` | CHAR(32) | NO | IDX | Enterprise customer UUID — links to LMS enterprise |
| `enterprise_name` | VARCHAR(255) | YES | — | Enterprise display name |
| `catalog_query_id` | INT | YES | FK | → `catalog_catalogquery.id` |
| `enabled_course_modes` | LONGTEXT (JSON) | NO | — | Allowed enrollment modes (e.g. `["verified"]`) |
| `publish_audit_enrollment_urls` | TINYINT(1) | NO | — | Whether to surface audit URLs |
| `created` | DATETIME(6) | NO | — | — |
| `modified` | DATETIME(6) | NO | — | — |

**Key relationship:**
```
catalog_enterprisecatalog.catalog_query_id → catalog_catalogquery.id
catalog_enterprisecatalog.uuid ← subscriptions_subscriptionplan.enterprise_catalog_uuid
```

**Multi-license relevance:** Each `SubscriptionPlan` references exactly one `EnterpriseCatalog`. Two plans referencing different catalogs = two different course sets for the same learner.

---

## 8. `catalog_contentmetadata`

**Django model:** `ContentMetadata`
**App:** `enterprise_catalog.apps.catalog`
**Role:** Stores cached JSON metadata for every course, course run, or program synced from Discovery service.

| Column | Type | Nullable | Key | Description |
|---|---|---|---|---|
| `id` | INT | NO | PK | Auto-increment ID |
| `content_key` | VARCHAR(255) | NO | UQ | Course key (e.g. `edX+DemoX`) or run key (`course-v1:edX+DemoX+2025`) |
| `content_type` | VARCHAR(255) | NO | — | `course`, `courserun`, `program` |
| `parent_content_key` | VARCHAR(255) | YES | IDX | For runs: parent course key; for programs: NULL |
| `_json_metadata` | LONGTEXT (JSON) | NO | — | Full Discovery API metadata blob |
| `created` | DATETIME(6) | NO | — | — |
| `modified` | DATETIME(6) | NO | — | IDX |

**M2M relationships:**
- `catalog_queries` → `catalog_catalogquery` **through** `catalog_contentmetadata_catalog_queries` (auto-managed Django M2M)
- `associated_content_metadata` → self (course ↔ program associations)

---

## 9. `catalog_contentmetadata_catalog_queries` (M2M Join Table)

**Role:** Django-auto-generated Many-to-Many join table linking content to catalog queries.
**This is the table that answers: "Which courses are in which catalog query?"**

| Column | Type | Nullable | Key | Description |
|---|---|---|---|---|
| `id` | INT | NO | PK | Auto-increment ID |
| `contentmetadata_id` | INT | NO | FK, IDX | → `catalog_contentmetadata.id` |
| `catalogquery_id` | INT | NO | FK, IDX | → `catalog_catalogquery.id` |

**Unique constraint:** `(contentmetadata_id, catalogquery_id)`

---

## 10. `catalog_restrictedcoursemetadata`

**Django model:** `RestrictedCourseMetadata`
**Role:** Per-catalog-query metadata overrides for courses that have restricted runs (B2B enterprise-restricted content). Separate from standard `ContentMetadata` to allow catalog-specific course run visibility.

| Column | Type | Nullable | Key | Description |
|---|---|---|---|---|
| `id` | INT | NO | PK | Auto-increment ID |
| `content_key` | VARCHAR(255) | NO | — | Course key (non-unique — one per catalog query) |
| `content_type` | VARCHAR(255) | NO | — | `course` |
| `parent_content_key` | VARCHAR(255) | YES | — | Parent course key |
| `_json_metadata` | LONGTEXT (JSON) | NO | — | Augmented metadata including restricted runs |
| `unrestricted_parent_id` | INT | YES | FK | → `catalog_contentmetadata.id` (standard version) |
| `catalog_query_id` | INT | YES | FK | → `catalog_catalogquery.id` |
| `created` | DATETIME(6) | NO | — | — |
| `modified` | DATETIME(6) | NO | — | — |

**Unique constraint:** `(content_key, catalog_query_id)`

---

## Full Join Path: User → Course Access

### Complete SQL (spans both service databases)

> **Note:** In production these are two separate databases (`license_manager` and `enterprise_catalog`).
> The join across services is logical — the `enterprise_catalog_uuid` is the bridging key.
> To run this as a single query in devstack you must JOIN across the two DBs explicitly
> or run separate queries and join in application code (as the BFF does).

**Step 1 — license-manager DB: Get all active licenses for the user**
```sql
-- Run against: license_manager database (port 3406)
SELECT
    l.uuid                          AS license_uuid,
    l.status,
    l.activation_date,
    ...
    sp.uuid                         AS plan_uuid,
    sp.title                        AS plan_title,
    sp.enterprise_catalog_uuid,     -- ← carry this to Step 2
    sp.expiration_date,
    ca.enterprise_customer_uuid,
    ca.enterprise_customer_slug
FROM subscriptions_license l
JOIN subscriptions_subscriptionplan sp  ON sp.uuid              = l.subscription_plan_id
JOIN subscriptions_customeragreement ca ON ca.uuid              = sp.customer_agreement_id
WHERE l.user_email                    = 'maria@knotion.com'
...
**Step 2 — enterprise-catalog DB: Find which courses are in those catalogs**
```sql
-- Run against: enterprise_catalog database
-- Replace (<cat-uuid-1>, <cat-uuid-2>) with results from Step 1
SELECT
    ec.uuid             AS catalog_uuid,
    ec.title            AS catalog_title,
    cq.id               AS catalog_query_id,
    cm.content_key      AS course_key,
    cm.content_type,
    cm.parent_content_key
FROM catalog_enterprisecatalog ec
JOIN catalog_catalogquery cq
    ON cq.id = ec.catalog_query_id
JOIN catalog_contentmetadata_catalog_queries cmcq
    ON cmcq.catalogquery_id = cq.id
JOIN catalog_contentmetadata cm
    ON cm.id = cmcq.contentmetadata_id
WHERE ec.uuid IN ('cat-data-science', 'cat-leadership')
  AND cm.content_type = 'course'
ORDER BY ec.uuid, cm.content_key;
```

**Step 3 — Course eligibility check (check a specific course)**
```sql
-- Is 'edX+DS101' in catalog 'cat-data-science'?
SELECT EXISTS (
    SELECT 1
    FROM catalog_enterprisecatalog ec
    JOIN catalog_catalogquery cq         ON cq.id = ec.catalog_query_id
    JOIN catalog_contentmetadata_catalog_queries cmcq ON cmcq.catalogquery_id = cq.id
    JOIN catalog_contentmetadata cm      ON cm.id = cmcq.contentmetadata_id
    WHERE ec.uuid          = 'cat-data-science'
      AND cm.content_key   = 'edX+DS101'
) AS course_in_catalog;
```
...
```
license-manager DB                          enterprise-catalog DB
══════════════════════════                  ══════════════════════════════════

subscriptions_customeragreement             catalog_catalogquery
  uuid (PK)                                   id (PK)
  enterprise_customer_uuid ──────────────►    uuid
  enterprise_customer_slug                    content_filter (JSON)
  default_enterprise_catalog_uuid ──────────► content_filter_hash
         │                                          │
         │ 1:N                                      │ 1:N (catalog_query_id FK)
         ▼                                          ▼
subscriptions_subscriptionplan              catalog_enterprisecatalog
  uuid (PK)                                   uuid (PK) ◄──────────────────────────────────────────────────┐
  title                                       title                                                         │
  start_date                                  enterprise_uuid                                               │
  expiration_date                             catalog_query_id FK                                           │
  enterprise_catalog_uuid ──────────────────────────────────────────────────────────────────────────────────┘
  customer_agreement_id FK                         │
  is_active                                        │ 1:N (via M2M join)
  should_auto_apply_licenses                       ▼
         │                                  catalog_contentmetadata_catalog_queries
         │ 1:N                                contentmetadata_id FK ──────────────────►  catalog_contentmetadata
         ▼                                    catalogquery_id FK                            id (PK)
subscriptions_license                                                                       content_key (UQ)
  uuid (PK)                                                                                 content_type
  status [UNASSIGNED|ASSIGNED|ACTIVATED|REVOKED]                                           parent_content_key
  user_email (PII) ─────────────────────────────────────────────────────────────────────►  _json_metadata (JSON)
  lms_user_id (PII)                                                   catalog_restrictedcoursemetadata
  subscription_plan_id FK                                               id (PK)
  activation_key                                                        content_key
  activation_date ◄── used by _select_best_license()                   catalog_query_id FK
  assigned_date                                                         unrestricted_parent_id FK
  revoked_date                                                          _json_metadata (JSON)
  auto_applied
         │
         │ 1:N
         ▼
subscriptions_licenseevent
  id (PK)
  license_id FK
  event_name
  event_data (JSON)
...
## Multi-License Table Interaction Summary

| Question | Tables Queried | Join Key |
|---|---|---|
| What plans does an enterprise have? | `customeragreement` → `subscriptionplan` | `customer_agreement_id` |
| What catalog does a plan cover? | `subscriptionplan` → `enterprisecatalog` | `enterprise_catalog_uuid` |
| What courses are in a catalog? | `enterprisecatalog` → `catalogquery` → `contentmetadata_catalog_queries` → `contentmetadata` | `catalog_query_id`, `catalogquery_id`, `contentmetadata_id` |
| Which licenses does a user have? | `license` WHERE `user_email` and `status='activated'` | `user_email` or `lms_user_id` |
| Which plan does each license belong to? | `license` → `subscriptionplan` | `subscription_plan_id` |
| Which catalog does each license grant access to? | `license` → `subscriptionplan.enterprise_catalog_uuid` → `enterprisecatalog` | cross-service UUID |
| Does License A grant access to Course X? | `license` → `subscriptionplan` → `enterprisecatalog` → `catalogquery` → `contentmetadata_catalog_queries` → `contentmetadata` WHERE `content_key = 'X'` | full chain |
| Build `licenses_by_catalog` (BFF) | `license` GROUP BY `enterprise_catalog_uuid` WHERE `status='activated'` AND `is_current=True` | `enterprise_catalog_uuid` |

---

## Devstack Connection Details

```bash
# license-manager DB
mysql -h 127.0.0.1 -P 3406 -u root -e "USE license_manager; ..."

# enterprise-catalog DB
mysql -h 127.0.0.1 -P 3406 -u root -e "USE enterprise_catalog; ..."

# List all subscription-related tables
mysql -h 127.0.0.1 -P 3406 -u root license_manager -e "SHOW TABLES LIKE 'subscriptions%';"

# List all catalog-related tables
mysql -h 127.0.0.1 -P 3406 -u root enterprise_catalog -e "SHOW TABLES LIKE 'catalog%';"

# Count licenses by status for an enterprise (multi-license audit)
mysql -h 127.0.0.1 -P 3406 -u root license_manager -e "
SELECT
    ca.enterprise_customer_slug,
    sp.title AS plan,
    sp.enterprise_catalog_uuid AS catalog_uuid,
    l.status,
    COUNT(*) AS count
FROM subscriptions_license l
JOIN subscriptions_subscriptionplan sp ON sp.uuid = l.subscription_plan_id
JOIN subscriptions_customeragreement ca ON ca.uuid = sp.customer_agreement_id
WHERE ca.enterprise_customer_slug = 'knotion'
GROUP BY ca.enterprise_customer_slug, sp.title, sp.enterprise_catalog_uuid, l.status
ORDER BY sp.title, l.status;
"
```
