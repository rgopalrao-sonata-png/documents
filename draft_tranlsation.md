...
10. [No-New-Table Option — Extend ContentTranslation](#10-no-new-table-option--extend-contenttranslation)
11. [Industry Filter Example — Skills Quiz](#11-industry-filter-example--skills-quiz)
...
### ADR-6: Extend `ContentTranslation` vs. New `TranslatedFacetCache` Table

**Decision:** Create a **new `TranslatedFacetCache` table** rather than adding `skill_names` / `subjects` JSON fields to the existing `ContentTranslation` model.
...
```mermaid
flowchart TD
    subgraph A["Option A — Extend ContentTranslation"]
        direction TB
        CT["ContentTranslation\ncontent_metadata_id + language_code\n+ title + description\n+ skill_names JSON  ADDED\n+ subjects JSON  ADDED"]
        COURSE1["course-ML  lang=fr\nskill_names: Apprentissage automatique, Python"]
        COURSE2["course-DS  lang=fr\nskill_names: Python, Statistiques"]
        COURSE3["course-AI  lang=fr\nskill_names: Python, Deep Learning"]
        CT --> COURSE1 & COURSE2 & COURSE3
        DUPE["⚠️ Python translated 3 times\nonce per course\nno deduplication"]
    end
    style A fill:#fff3e0,stroke:#E65100
    style DUPE fill:#ffebee,stroke:#f44336
```

**Pros:**
- Zero schema discovery cost — existing model, admin, migrations pattern all reusable
- `create_localized_algolia_object` already reads `ContentTranslation` — skills/subjects become more fields on the same row
- Single DB read fetches title + skills + subjects together
- Additive migration — rollback is trivial (null columns)
- Works fine at small scale (< 5,000 courses, < 500 unique skills)

**Cons:**

| Problem | Real-world impact |
|---|---|
| No deduplication — `"Python"` stored once per course per language | 10,000 courses × 20 locales = **200,000 rows** storing the same word repeatedly |
| Xpert AI re-translates same skill per course | `"Python"` at 10,000 courses = 10,000 API calls per locale, every populate run |
| Cannot fix one skill globally | A mistranslation of `"Machine Learning"` requires updating thousands of JSON blobs |
| No per-skill source tracking | Human-corrected values get overwritten on next populate run |
| JSON arrays are opaque | Cannot efficiently query "which courses have French translation for skill X" |
| Hard to migrate later | Extracting JSON blobs back into a proper table is costly when catalog grows |

#### Option B — New `TranslatedFacetCache` Table (Recommended)

Keyed on `(english_value, field_type, language_code)` — one row per unique value per locale.

```mermaid
flowchart TD
    subgraph B["Option B — TranslatedFacetCache"]
        direction TB
        TFC["TranslatedFacetCache\nenglish_value + field_type + language_code\nPython + skill + fr → Python\nStored ONCE regardless of how many courses have it"]
        C1["course-ML uses Python → lookup TFC"]
        C2["course-DS uses Python → lookup TFC"]
        C3["course-AI uses Python → lookup TFC"]
        TFC --> C1 & C2 & C3
        WIN["✅ Python translated ONCE per locale\n1 Xpert AI call, reused by 10,000 courses"]
    end
    style B fill:#e8f5e9,stroke:#388E3C
    style WIN fill:#e8f5e9,stroke:#388E3C
```

**Pros:**
- Each unique value translated **once per locale, ever** — regardless of how many courses share it
- Fix one mistranslation with a single row UPDATE — immediately reflected on next reindex
- `source` field distinguishes `xpert_ai` vs `human` — human corrections are never overwritten
- Clean indexed query: `WHERE english_value IN (...) AND field_type='skill' AND language_code='fr'`
- Directly supports future `Ability` and `Industry` facets by adding new `field_type` choices — no schema change
- Straightforward migration path to the normalized `FacetValue + FacetValueTranslation` schema in Section 9

**Cons:**
- One additional migration and Django model to write (~1–2 days extra)
- Slightly more complex `populate_translations` command (collect unique values first, then translate)
- `create_localized_algolia_object` requires two DB reads (one for `ContentTranslation`, one for `TranslatedFacetCache`) instead of one

#### Concrete Numbers

Assumptions: 100,000 content items, 5,000 unique skill values, 20 locales, average skill appears in 50 courses.

| Metric | Extend `ContentTranslation` | `TranslatedFacetCache` |
|---|---|---|
| Xpert AI calls (initial bootstrap) | 100,000 × 20 = **2,000,000** | 5,000 × 20 = **100,000** |
| Xpert AI calls (weekly incremental, 100 new skills) | 100 × 50 courses × 20 = **100,000/week** | 100 × 20 = **2,000/week** |
| DB rows for skill translations | 100,000 × 20 = **2,000,000 JSON blobs** | 5,000 × 20 = **100,000 rows** |
| Fix one mistranslation | UPDATE ~50–100 rows | UPDATE **1 row** |
| Storage per locale added | ~500 MB JSON at scale | ~5 MB |

#### Pragmatic Middle Path (if new table is blocked)

If the team cannot create a new table in the current sprint, extend `ContentTranslation` with a **session-level in-memory deduplication** during the populate run to avoid the worst-case Xpert AI call explosion:

```python
# In populate_translations — partial mitigation WITHOUT a new table
# Saves API calls within one run, but NOT across runs
translation_session_cache: dict[tuple, str] = {}  # {(value, lang): translated}

for skill in course_skills:
    key = (skill, language_code)
    if key not in translation_session_cache:
        translation_session_cache[key] = xpert_ai_translate(skill, language_code)
    translated = translation_session_cache[key]
```

This is a **stopgap only** — re-translation still happens on every weekly cron run. Add a `# TODO: replace with TranslatedFacetCache` comment and schedule the proper table for the next sprint.

#### Decision Rationale

- **2 extra days** of implementation work to create `TranslatedFacetCache`
- Saves **20× Xpert AI cost** from day one of the bootstrap run
- Saves **20× storage** at catalog scale
- Provides clean correctability and auditability that JSON blobs cannot offer
- The pragmatic path only makes sense as a time-boxed interim, not as a permanent design

**Verdict: Create the new table. Reject the JSON field extension as a permanent solution.**
...
## 10. No-New-Table Option — Extend ContentTranslation

> This section captures the full analysis of implementing skills/subject translation **without creating any new DB tables**, using only the existing `ContentTranslation` model. Refer to ADR-6 for the decision rationale.

### What `ContentTranslation` currently stores

```python
# models.py L1886 — existing fields
class ContentTranslation(TimeStampedModel):
    content_metadata  # FK to ContentMetadata
    ...
    prerequisites
    subtitle
    source_hash       # SHA256 of source for change detection
...
```python
# Migration: add two nullable JSON fields
skill_names_translated = models.JSONField(
    null=True, blank=True,
    help_text=(
        "Translated skill_names array for Algolia indexing. "
        "INTERIM: to be replaced by TranslatedFacetCache. "
        "Do not build deduplication logic against this field."
    )
)
subjects_translated = models.JSONField(
    null=True, blank=True,
    help_text="Translated subjects array. Same migration note as skill_names_translated."
)
```

### How `create_localized_algolia_object` would use it
...
def create_localized_algolia_object(algolia_object, content_metadata, language_code):
    ...
    _apply_text_translation(localized, translation)

    # Use pre-stored translated arrays directly — no cache lookup needed
    if translation.skill_names_translated:
        localized['skill_names'] = translation.skill_names_translated
    if translation.subjects_translated:
        localized['subjects'] = translation.subjects_translated
...
### Populate command change

```python
def _populate_facets_for_content(self, content_metadata, language_code, session_cache):
    """
    Translate skill_names and subjects for one ContentMetadata row.
    Uses session_cache to avoid re-translating within one command run.
    Does NOT deduplicate across runs — same skill re-translated weekly.
    """
    skills = (content_metadata.json_metadata or {}).get('skill_names', [])
    subjects = (content_metadata.json_metadata or {}).get('subjects', [])

    translated_skills = [
        session_cache.setdefault(
            (s, language_code),
            self._batch_translate([s], language_code)[0]
        )
        for s in skills
    ]
    translated_subjects = [
    ...
    ContentTranslation.objects.update_or_create(
        ...
        language_code=language_code,
        defaults={
            'skill_names_translated': translated_skills,
            'subjects_translated': translated_subjects,
        }
    )
```
...
| Criterion | Extend `ContentTranslation` | `TranslatedFacetCache` |
|---|---|---|
| New DB table required | ❌ No | ✅ Yes (1 model, 1 migration) |
| Implementation time | ~0.5 days | ~2 days |
| Xpert AI calls (100k courses, 20 locales) | ~2,000,000 | ~100,000 |
| Weekly incremental cost (100 new skills) | ~100,000 calls/week | ~2,000 calls/week |
| Storage for skill translations | ~2M JSON blobs | ~100K rows |
| Fix one mistranslation | Update 50–100 rows | Update 1 row |
| Human override protection | ❌ Overwritten on next run | ✅ `source=human` protected |
| Supports future facet types | ❌ New JSON fields per type | ✅ New `field_type` value |
| Migration path to normalized schema | ❌ JSON blob extraction needed | ✅ Direct ALTER TABLE |

**Recommendation:** Use `TranslatedFacetCache`. If blocked, use the `ContentTranslation` extension only as a time-limited interim (one sprint) with the session-level cache mitigation, with a committed ticket to migrate.

---

## 11. Industry Filter Example — Skills Quiz
...
*Document version: 2.1 | April 2026 | v2.1: Added ADR-6 comparing ContentTranslation extension vs TranslatedFacetCache with concrete cost numbers, added Section 10 with full no-new-table implementation guide and summary comparison table, renumbered Industry Filter as Section 11.*
