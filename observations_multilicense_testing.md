...
**Date:** April 2, 2026
**Version:** 2.0
**Status:** Proposed Architecture (Revised)
**Scope:** Flowcharts, Component Design, Code Implementation, Expiration Continuity, Operational Safeguards, Tests
...
8. [Review Resolutions](#review-resolutions-alex-dusenbery-v1-review--2026-03-30)
9. [Subscription Expiration Continuity](#subscription-expiration-continuity-subplan-problem)
10. [Operational Safeguards and Rollout Recommendations](#operational-safeguards-and-rollout-recommendations)
11. [Updated Migration Path](#updated-migration-path-v2)
12. [Open Questions](#open-questions-v2-addenda)
...
There is a **6th bottleneck in `enterprise_access/apps/bffs/context.py`**: `_initialize_secured_algolia_api_keys()` calls `get_and_cache_secured_algolia_search_keys()` which fetches the Algolia key scoped to **all** `available_subscription_catalogs` in the customer agreement — not to only the catalogs the learner actually holds active licenses for. This means `catalog_uuids_to_catalog_query_uuids` and the resulting `secured_algolia_api_key` are over-permissive: a learner can search courses in catalogs they have no license for. **Fix:** after `load_and_process_subscription_licenses()` completes, the context must re-scope `catalog_uuids_to_catalog_query_uuids` (and regenerate the Algolia key) to only the catalog UUIDs present in the learner's own activated license plans.
...
graph TB
    ...
    LM[License Manager<br/>━━━━━━━━━━━━━━<br/>Store & return ALL licenses<br/>No blocking API changes ✅]
    end
...
| No blocking License Manager API changes | Minimal blast radius; faster delivery while still allowing reuse of an existing expiration event or a minimal event emission addition |

---

## Detailed Component Design

### Component 1: License Manager (No Blocking API Changes)

**Status:** ✅ Already works correctly for license retrieval — returns ALL licenses.

**Note:** The proposed expiration-continuity design assumes either:
1. an existing plan-expired event can be consumed by `enterprise-access`, or
2. a minimal event emission is added without changing the License Manager read API contract.
...
def _map_courses_to_licenses(self, enrollment_intentions):
   ...
   2. If multiple match, apply deterministic tie-breaker.
    ...
    """
    # NOTE: `current_activated_licenses` is an existing @property on BaseLearnerPortalHandler
    # (defined in handlers.py). Do NOT introduce a separate _get_current_activated_licenses() method.
...
### Component 2.5: BFF Context — Algolia / `catalog_uuids_to_catalog_query_uuids` (Bottleneck #6)

**File:** `enterprise_access/apps/bffs/context.py`

**Root cause:** `_initialize_secured_algolia_api_keys()` is called during context initialization (before subscription licenses are loaded). It calls `get_and_cache_secured_algolia_search_keys()` → `EnterpriseCatalogUserV1ApiClient.get_secured_algolia_api_key()`, which scopes the Algolia key to **all** catalogs listed in the customer agreement's `available_subscription_catalogs`. This is independent of which catalogs the learner actually holds active licenses for.

**Symptom observed:** A learner with 3 activated licenses (catalogs `11111111`, `22222222`, `33333333`) receives an Algolia key filtered to all 5 catalogs in the agreement (`11111111`–`55555555`), granting them search access to courses they cannot enroll in.

**Secondary symptom:** When a fix incorrectly scopes `catalog_uuids_to_catalog_query_uuids` to only the "primary" `subscription_license` (singular), the Algolia key generator receives only 1 catalog UUID and falls back to `null`, which causes the frontend to use the unfiltered base Algolia key — granting completely unrestricted search access.

**Current code (context.py ~line 367):**
```python
def _initialize_secured_algolia_api_keys(self):
    secured_algolia_api_key_data = get_and_cache_secured_algolia_search_keys(
        self.request, self._enterprise_customer_uuid,
    )
    # Returns key scoped to ALL available_subscription_catalogs ❌
    (secured_algolia_api_key, catalog_uuids_to_catalog_query_uuids, valid_until) = \
    ...
    self.data.update({
        ...
        'algolia': {'secured_algolia_api_key': secured_algolia_api_key, 'valid_until': valid_until}
    })
```

**Required fix — re-scope after licenses are loaded:**

After `load_and_process_subscription_licenses()` finishes (inside `load_and_process()` in `BaseLearnerPortalHandler`), filter `catalog_uuids_to_catalog_query_uuids` to only the catalogs for which the learner holds a current, activated license, then regenerate/filter the Algolia key:

```python
def _rescope_algolia_to_activated_license_catalogs(self):
    """
    ✅ NEW: Narrow catalog_uuids_to_catalog_query_uuids (and thus the Algolia key)
    to only those catalogs backed by the learner's own current, activated licenses.

    Must be called AFTER load_and_process_subscription_licenses().
    """
    activated_catalog_uuids = {
        lic['subscription_plan']['enterprise_catalog_uuid']
        for lic in self.current_activated_licenses
        if lic.get('subscription_plan', {}).get('enterprise_catalog_uuid')
    }
    if not activated_catalog_uuids:
        # No activated licenses → no Algolia access; null key is correct
        self.context.data['catalog_uuids_to_catalog_query_uuids'] = {}
        self.context.data['algolia'] = {'secured_algolia_api_key': None, 'valid_until': None}
        return

    full_catalog_map = self.context.data.get('catalog_uuids_to_catalog_query_uuids', {})
    scoped_catalog_map = {
        k: v for k, v in full_catalog_map.items()
        if k in activated_catalog_uuids
    }
    self.context.data['catalog_uuids_to_catalog_query_uuids'] = scoped_catalog_map
    # The Algolia key was generated server-side from the full set; it must be regenerated
    # from catalog-service with the scoped catalog list, OR the catalog service endpoint
    # must accept a catalog_uuids filter parameter.
    # ⚠️ OPEN: Coordinate with enterprise-catalog team on whether
    # get_secured_algolia_api_key can accept a catalog_uuids param so the key
    # itself is correctly scoped, or whether a second call is needed here.
```

**Correct output for Alice (3 activated licenses):**
```json
"catalog_uuids_to_catalog_query_uuids": {
...
```
Not 5 (all agreement catalogs), and not 1 (only the primary license catalog).
...
const useUserSubsidyApplicableToCourse = () => {
  ...
  const multiLicenseEnabled = getConfig().FEATURES?.ENABLE_MULTI_LICENSE_ENTITLEMENTS ?? false;
...
class TestMapCoursesToLicenses(TestCase):
    ...
    @mock.patch.object(BaseLearnerPortalHandler, 'current_activated_licenses', new_callable=mock.PropertyMock)
...
| License A expires, License B active | 2 licenses, License A expiring | Re-upgraded to verified via License B | Expiration continuity |
| Expiration modal, 2 licenses | License A 30 days, License B 6 months | Warning shows License A's date | Modal logic |
| Search with 2 catalogs | License A (cat-A), License B (cat-B) | Both catalog UUIDs in search scope | Search scoping |

---

## Review Resolutions (Alex Dusenbery, v1 Review — 2026-03-30)

The following sections address all high-priority gaps identified in the v1 review.

---

### R1. Feature Flag Pattern Fix

Alex noted the spec incorrectly used `features.ENABLE_*`. The repo convention is
`getConfig().FEATURES.*` for env-based rollout flags.

**Correct pattern (MFE):**
```javascript
// ✅ CORRECT — use getConfig()
import { getConfig } from '@edx/frontend-platform/config';

const multiLicenseEnabled = getConfig().FEATURES?.ENABLE_MULTI_LICENSE_ENTITLEMENTS ?? false;
```

All occurrences of `features.ENABLE_MULTI_LICENSE_ENTITLEMENTS` in the MFE must
be replaced with the above pattern.

---

### R2. TypeScript Type Definitions

Add to `src/types/types.d.ts`:

```typescript
type LicensesByCatalog = Record<string, SubscriptionLicense[]>;

type SubscriptionsDataV2 = {
  /** Collection-first (canonical) */
  subscriptionLicenses: SubscriptionLicense[];
  subscriptionLicensesByStatus: {
    activated: SubscriptionLicense[];
    assigned: SubscriptionLicense[];
    revoked: SubscriptionLicense[];
  };
  /** Catalog UUID → activated, current licenses index for O(1) lookups */
  licensesByCatalog: LicensesByCatalog | null;
  /** 'v1' = single-license legacy, 'v2' = multi-license */
  licenseSchemaVersion: 'v1' | 'v2';
  /** @deprecated Use subscriptionLicenses[]. Retained for backward compatibility. */
  subscriptionLicense: SubscriptionLicense | null;
  /** @deprecated Use subscriptionLicenses[0].subscriptionPlan. */
...
### R3. Search Catalog Scoping

**File:** `src/components/app/data/hooks/useSearchCatalogs.js` (line ~16)

**Current (broken for multi-license):**
```javascript
// BROKEN: Only adds first license's catalog
if (subscriptionLicense?.subscriptionPlan.isCurrent
    && subscriptionLicense?.status === LICENSE_STATUS.ACTIVATED) {
...
**Fix:**
```javascript
// FIXED: Add ALL activated, current licenses' catalog UUIDs to search scope
const multiLicenseEnabled = getConfig().FEATURES?.ENABLE_MULTI_LICENSE_ENTITLEMENTS ?? false;

if (multiLicenseEnabled && subscriptionLicenses?.length) {
  subscriptionLicenses
    .filter(lic => lic.status === LICENSE_STATUS.ACTIVATED && lic.subscriptionPlan?.isCurrent)
    .forEach(lic => {
      if (lic.subscriptionPlan?.enterpriseCatalogUuid) {
        catalogUUIDs.add(lic.subscriptionPlan.enterpriseCatalogUuid);
      }
    });
} else if (subscriptionLicense?.subscriptionPlan?.isCurrent
           && subscriptionLicense?.status === LICENSE_STATUS.ACTIVATED) {
  catalogUUIDs.add(subscriptionLicense.subscriptionPlan.enterpriseCatalogUuid);
}
```
...
### R4. Cache Invalidation on License Activation

**File:** `src/components/app/routes/data/utils.js` (line ~63-101)

When a license is activated, the root loader must:
1. Append the new license to `subscriptionLicenses` in sorted order.
2. Rebuild `licensesByCatalog` index.
3. Keep the deprecated `subscriptionLicense` singular in sync.

```javascript
export function patchSubscriptionCacheOnActivation(queryClient, queryKey, newlyActivatedLicense) {
  queryClient.setQueryData(queryKey, (oldData) => {
    if (!oldData) return oldData;
    const existing = oldData.subscriptionLicenses ?? [];
    if (existing.find(l => l.uuid === newlyActivatedLicense.uuid)) return oldData;

    const updated = [...existing, newlyActivatedLicense].sort((a, b) => {
      if (a.subscriptionPlan.isCurrent !== b.subscriptionPlan.isCurrent) {
        return a.subscriptionPlan.isCurrent ? -1 : 1;
      }
      return new Date(b.subscriptionPlan.expirationDate) - new Date(a.subscriptionPlan.expirationDate);
    ...
    return {
      ...oldData,
      subscriptionLicenses: updated,
      licensesByCatalog: buildCatalogIndex(updated),
      subscriptionLicense: updated[0] ?? null,
      subscriptionPlan: updated[0]?.subscriptionPlan ?? null,
    };
  });
}
```
...
### R5. Expiration Modal — Which License Triggers Warning?

**Decision:** Show the modal for the license with the **soonest** expiration date
among activated `isCurrent` licenses. If another license has 6+ months remaining,
include that context in the modal copy so the learner understands they retain
verified access for longer courses covered by that license.

```javascript
function useExpirationModalLicense(subscriptionLicenses) {
  return useMemo(() => {
    const currentActivated = (subscriptionLicenses ?? []).filter(
      lic => lic.status === LICENSE_STATUS.ACTIVATED && lic.subscriptionPlan?.isCurrent
    );
    if (!currentActivated.length) return null;
    return [...currentActivated].sort(
      (a, b) => new Date(a.subscriptionPlan.expirationDate) - new Date(b.subscriptionPlan.expirationDate)
    )[0];
  }, [subscriptionLicenses]);
}

// In modal component:
const expiringLicense = useExpirationModalLicense(subscriptionLicenses);
const otherActiveLicenses = (subscriptionLicenses ?? []).filter(
  lic => lic.uuid !== expiringLicense?.uuid
    && lic.status === LICENSE_STATUS.ACTIVATED
    && lic.subscriptionPlan?.isCurrent
);
// If otherActiveLicenses.length > 0: render note "You have other active licenses"
```

---

### R6. `shouldUpgradeUserEnrollment` Multi-License Update
...
export function shouldUpgradeUserEnrollment({
  userEnrollment,
  subscriptionLicenses,  // NEW: array instead of singular
  catalogsWithCourse,
  enrollmentUrl,
}) {
  if (!isEnrollmentUpgradeable(userEnrollment) || !enrollmentUrl) return false;
  const applicableLicenses = getApplicableLicensesForCourse(subscriptionLicenses, catalogsWithCourse);
  return !!selectBestLicense(applicableLicenses);
}
```

---

## Subscription Expiration Continuity (Subplan Problem)
...
| Enrollment timing | License A status at expiry | License B exists? | Outcome with fix |
|---|---|---|---|
| Enrolled after flag ON | License B used at enrollment | Yes | No downgrade (preventive) |
| Enrolled before flag ON | License A used at enrollment | Yes, covers same course | Remediation task re-upgrades |
| Enrolled before flag ON | License A used at enrollment | No other license | Stays in audit (correct) |
| No multi-license scenario | Only License A | No | Legacy behavior unchanged |

---

## Operational Safeguards and Rollout Recommendations

To make the expiration-remediation design production-safe, the implementation should adopt the repository's existing Celery patterns for idempotent, retryable background work.

### Recommended safeguards

1. **Idempotent remediation task**
  - Key the task by `(enterprise_customer_uuid, learner_id, course_run_key, expired_plan_uuid)`.
  - Before re-upgrading, check whether the enrollment is already verified or was already remediated for the same expired plan.

2. **Retryable, bounded execution**
  - Use the project's retry/task base patterns instead of a bare Celery task where practical.
  - Retry transient LMS / License Manager errors, but cap retries and emit a terminal failure metric.

3. **Auditability**
  - Record the old plan UUID, new license UUID, learner ID, course key, and remediation timestamp.
  - This can be app-level structured logging first, with a durable model only if operations needs queryable history.

4. **Fallback reconciliation job**
  - Add a daily reconciliation command or task that scans for recent downgraded enrollments and retries missed remediations.
  - This protects against dropped events or temporary downstream outages.

5. **Observability**
  - Emit counters for: affected learners, remediated enrollments, no-match cases, retries, and permanent failures.
  - Add enterprise/customer dimensions where available for rollout monitoring.

### Recommended ownership decision

**Preferred approach:** keep remediation in `enterprise-access`, triggered asynchronously by an expiration event.

**Why this is preferred:**
- keeps entitlement-recovery logic close to the BFF and enterprise enrollment integrations,
- limits blast radius in License Manager,
- supports phased rollout and rollback independent of core expiration processing,
- aligns with the repo's Celery and service-client patterns.

---

## Updated Migration Path (v2)
...
| 6 | MFE modal | Expiration modal soonest-license logic |
| 7 | Rollout | Enable BFF Waffle flag for pilot enterprise |
| 8 | Rollout | Enable MFE flag for pilot enterprise |
| 9 | Stabilize | Monitor for 1-2 months in production |
| 10 | Deprecation | Remove singular `subscription_license` legacy fields |
